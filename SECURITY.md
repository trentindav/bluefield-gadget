...
The release model of the BlueField-3 snap is following rolling releases. 

The bluefield3-gadget snap is a gadget snap that contains the bootloader assets required for booting
...
carefully. Most of the files published with this snap are sourced from the Ubuntu and NVIDIA
archives and as such must adhere to same security policies as established for upstream.

## Supported versions
When reporting security issues against the bluefield3-gadget snap, only the latest 
...
Any vulnerability that allows the bluefield3-gadget snap to interfere outside of the
intended restrictions qualifies as a security issue, including vulnerabilities that
...
[GitHub](TODO: final repository location). See
